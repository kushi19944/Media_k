...
const AJA_URL = process.env.New_AJA_URL;
const AJA_ID = process.env.New_AJA_ID;
const AJA_PW = process.env.New_AJA_PW;
...
async function AJALogin() {
  try {
    await RPA.WebBrowser.get(AJA_URL);
        ...
        if (IDText == SheetData[2]) {
          ...
          for (let j = 0; j < 5; j++) {
            try {
              ...
              await RPA.Logger.info(`【入札調整】値:${SheetData[3]}`);
              await YenInput.clear();
              await RPA.sleep(300);
              await RPA.WebBrowser.sendKeys(YenInput, [`${SheetData[3]}`]);
              await RPA.sleep(200);
              await RPA.WebBrowser.sendKeys(YenInput, [
                await RPA.WebBrowser.Key.ENTER,
              ]);
              await RPA.sleep(800);
              // 値を変更するとポップアップが出現するようになったため対応できるように変更
              try {
                const PopUP = await RPA.WebBrowser.wait(
                  RPA.WebBrowser.Until.elementLocated({
                ...
                await RPA.WebBrowser.driver.executeScript(
                  `document.getElementsByClassName('modal-content')[0].children[0].children[3].children[1].children[0].click()`
                );
                await RPA.Logger.info(
                  `【入札調整】ポップアップアラートクリックしました`
                );
              } catch {
                RPA.Logger.info(`【入札調整】ポップアップアラート無し`);
              }
              ...
              if (j == 4) {
                await RPA.Logger.info('【入札調整】値入力失敗');
                await SetValues_Function(SheetWorkingRow, `入力失敗`);
                break;
              }
            }
          }
